...
### 1. DATA IMPORT AND PREPARATION
cg <- read_excel("data/elevation/app_cg_2023.xlsx")
...
fac_list <- c("pop", "pop.group", "label.ID", "block", "plant", "new.ID", "full.garden", "garden.group", "stage")
num_list<- c("live", "herb", "fl.fr", "buds", "stem.d")
...
# # write data
write.csv(cg.herb, "data/elevation/elev_rel_abs_NO_HERB.csv")
write.csv(cg, "data/elevation/elev_rel_abs_HERB.csv")
...
# write
write.csv(Hedge.NOHERB, "data/elevation/elev_hedgesD_NO_HERB.csv")
...
# write
write.csv(Hedge.HERB, "data/elevation/elev_hedgesD_HERB.csv")
